...
def detect_lab(text):
    ...
    if re.search(r'INV-D[0-9]', text):
        return "Dent8"
    if re.search(r'INV-IN[0-9]', text):
...
def parse_hall(text, tables, lab):
    ...
    rows = []
    # Hall format: Tax Date  Job  Your ref  Description  Patient  Orig Amt  Balance
    # Lines look like: 24/02/26 T61947 - INV68272 U BUKSH 3900.00 3078.50
    pattern = re.compile(
